...
describe("MerklePayout", async function () {
  ...
  describe("#constructor", () => {
    it("deploys properly", async function () {
      ...
      payout = await MerklePayout.deploy(
        token.address,
        RANDOM_BYTES32,
        funder.address,
        overrides
      ...
      // Verify deploy
      expect(isAddress(payout.address), "Failed to deploy MerklePayout").to.be
        .true;
  ...
  describe("#claim", () => {
    ...
    it("it fails for empty proof / invalid index", async () => {
      const [funder, claimee] = await ethers.getSigners();
    ...
    describe("two account balance tree", () => {
      ...
      it("successful claim", async () => {
        ...
        const merkleProof0 = tree.getProof(
          0,
          claimee0.address,
          BigNumber.from(100)
        );
        const claimArgs0 = {
          index: 0,
          claimee: claimee0.address,
          amount: 100,
          merkleProof: merkleProof0,
        };

        await expect(payout.claim(claimArgs0))
          .to.emit(payout, "FundsClaimed")
          .withArgs(0, claimee0.address, 100);

        // second claimee claims
        const merkleProof1 = tree.getProof(
          1,
          claimee1.address,
          BigNumber.from(101)
        );
        const claimArgs1 = {
          index: 1,
          claimee: claimee1.address,
          amount: 101,
          merkleProof: merkleProof1,
        };

        await expect(payout.claim(claimArgs1))
          .to.emit(payout, "FundsClaimed")
          .withArgs(1, claimee1.address, 101);
      });
      ...
      it("must have enough tokens to transfer", async () => {
        ...
        await expect(payout.claim(claimArgs0)).to.be.revertedWith(
      ...
      it("cannot allow 2 claims", async () => {
        ...
        await expect(payout.claim(claimArgs0)).to.be.revertedWith(
          "MerklePayout: Funds already claimed."
        );
      });
      ...
      it("claimee1 cannot claim second time after claimee0 has claimed", async () => {
        ...
        await expect(payout.claim(claimArgs1)).to.be.revertedWith(
          "MerklePayout: Funds already claimed."
        );
      });
  ...
  describe("#batchClaim", () => {
    ...
    it("BatchClaimTriggered fired on successful batch claim", async () => {
      ...
      await expect(batchClaim)
        .to.emit(payout, "BatchClaimTriggered")
    ...
    it("user who is not funder cannot call batch claim", async () => {
      ...
      await expect(
        payout.connect(claimee0).batchClaim([claimArgs0, claimArgs1])
      ).to.be.revertedWith("MerklePayout: caller is not the funder");
    });
  ...
  describe("#reclaimFunds", () => {
    ...
    it("reclaimFunds invoked by another addrees", async () => {
      ...
      await expect(payout.reclaimFunds(token.address)).to.be.revertedWith(
        "MerklePayout: caller is not the funder"
      );
    });
    ...
    it("reclaimFunds emits ReclaimFunds event before any claim", async () => {
      expect(await payout.reclaimFunds(token.address))
        .to.emit(payout, "ReclaimFunds")
        .withArgs(funder.address, token.address, 201);
    });
